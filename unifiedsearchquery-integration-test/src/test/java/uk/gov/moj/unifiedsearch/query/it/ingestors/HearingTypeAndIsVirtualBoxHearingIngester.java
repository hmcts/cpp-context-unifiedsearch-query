...
public class HearingTypeAndIsVirtualBoxHearingIngester {
    ...
    private List<CaseDocument> caseListForPartyDateOfBirthAndNameSearch() {
        ...
        caseBuilderList.get(0).withHearings(asList(createHearing(SEARCHED_HEARING_TYPE_UUID, true),
                createHearing("222505b6-4a9e-4820-9ddc-f9a627914ef0", false),
                createHearing("333505b6-4a9e-4820-9ddc-f9a627914ef0", true)));

        caseBuilderList.get(1).withHearings(singletonList(createHearing(SEARCHED_HEARING_TYPE_UUID, false)));

        caseBuilderList.get(2).withHearings(singletonList(createHearing("222505b6-4a9e-4820-9ddc-f9a627914ef0", false)));
    ...
    public List<CaseDocument> getNonVirtualBoxHearingSearchedCases() {
        return asList(indexedCaseList.get(1));
    }

    private static HearingDocument.Builder createHearing(final String hearingTypeUuid, final boolean isVirtualBoxHearing) {
        final HearingDocument.Builder hearingBuilder = defaultHearingAsBuilder();
        hearingBuilder.withHearingTypeId(hearingTypeUuid);
        hearingBuilder.withVirtualBoxHearing(isVirtualBoxHearing);
