...
public class AssertCases {
    ...
    public static void assertCases(final List<Case> actualCases, final List<CaseDocument> expectedCaseDocuments) {
        ...
        assertThat(actualCases, hasSize(lessThanOrEqualTo(expectedCaseDocuments.size())));

        for (final CaseDocument expectedCaseDocument : expectedCaseDocuments) {

            final Predicate<? super Case> sameCaseIds = aCase -> aCase.getCaseId().equals(fromString(expectedCaseDocument.getCaseId()));
